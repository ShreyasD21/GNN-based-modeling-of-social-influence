...
class GNNModel:
    def __init__(self, f_in, hand_dim, hidden=16, use_handcrafted=True):
        ...
        lim = lambda a, b: np.sqrt(6 / (a + b))
        self.W1 = rng.uniform(-lim(f_in, hidden), lim(f_in, hidden), (f_in, hidden)); self.b1 = np.zeros(hidden)
        self.W2 = rng.uniform(-lim(hidden, hidden), lim(hidden, hidden), (hidden, hidden)); self.b2 = np.zeros(hidden)
        head_in = hidden * 2 + (hand_dim if use_handcrafted else 0)
        self.W3 = rng.uniform(-lim(head_in, hidden), lim(head_in, hidden), (head_in, hidden)); self.b3 = np.zeros(hidden)
        self.W4 = rng.uniform(-lim(hidden, 1), lim(hidden, 1), (hidden, 1)); self.b4 = np.zeros(1)
    ...
    def forward(self, inst, hand_feat):
        ...
        AX1 = A @ X;              Z1 = AX1 @ self.W1 + self.b1; H1 = self._relu(Z1)
        AX2 = A @ H1;             Z2 = AX2 @ self.W2 + self.b2; H2 = self._relu(Z2)
        ...
        fused = np.concatenate([readout, hand_feat]) if self.use_handcrafted else readout

        z3 = fused @ self.W3 + self.b3; a3 = self._relu(z3)
    ...
    def backward(self, cache, y_true, lr):
        ...
        dW4 = np.outer(cache["a3"], dz4); db4 = dz4
        da3 = dz4 @ self.W4.T
        dz3 = da3 * (cache["z3"] > 0)
        dW3 = np.outer(cache["fused"], dz3); db3 = dz3
        ...
        dmean = np.sum(dHnr * -1.0 / std, axis=0, keepdims=True) + dvar * np.mean(-2.0 * x_mu, axis=0, keepdims=True)
        dH2 = np.zeros_like(cache["H2"])
        dH2[:N] = dHnr / std + dvar * 2.0 * x_mu / N + dmean / N

        dZ2 = dH2 * (cache["Z2"] > 0)
        dW2 = cache["AX2"].T @ dZ2; db2 = dZ2.sum(0)
        dH1 = (cache["A"].T @ (dZ2 @ self.W2.T))

        dZ1 = dH1 * (cache["Z1"] > 0)
        dW1 = cache["AX1"].T @ dZ1; db1 = dZ1.sum(0)

        grads = dict(W1=dW1, b1=db1, W2=dW2, b2=db2, W3=dW3, b3=db3, W4=dW4, b4=db4)
        for k, g in grads.items():
            setattr(self, k, self._adam_step(getattr(self, k), g, self._adam[k], lr))

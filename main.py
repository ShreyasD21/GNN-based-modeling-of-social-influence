...
def main():
    ...
    train_v, val_v, test_v = eligible[:n_tr], eligible[n_tr:n_tr + n_va], eligible[n_tr + n_va:]
    ...
    def build(vs):
        ...
        hand = np.array([handcrafted_features(G, v, y, attrs, clustering) for v in vs])
        return inst, hand
    ...
    plain = GNNModel(f_in=2, hand_dim=train_hand.shape[1], use_handcrafted=False)
    train(plain, train_inst, train_hand, y_train, epochs=15)
    plain_proba = [plain.forward(i, h)[0] for i, h in zip(test_inst, test_hand)]
